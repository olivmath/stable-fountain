...
int64_t hook(uint32_t reserved)
{
    ...
    {
        vault_pusd = float_sto_set((uint32_t)vault, 8);
        vault_xrp  = float_sto_set((uint32_t)(vault + 8), 8);
        vault_exists = 1;
    }
    ...
    if (is_xrp)
    {
        ...
        if (float_compare(pusd_to_send, 0, COMPARE_LESS))
        {
            ...
            {
                if (float_sto((uint32_t)(vault + 8), 8, 0,0,0,0, vault_xrp, -1) != 8)
        ...
        if (float_sto((uint32_t)vault, 8, 0,0,0,0, vault_pusd, -1) != 8 ||
            float_sto((uint32_t)(vault + 8), 8, 0,0,0,0, vault_xrp, -1) != 8)
            rollback(SBUF("Peggy: Internal error writing vault"), 1);
    ...
    else
    {
        ...
        // set / update the vault
        if (float_sto((uint32_t)vault, 8, 0,0,0,0, vault_pusd, -1) != 8 ||
            float_sto((uint32_t)(vault + 8), 8, 0,0,0,0, max_vault_xrp, -1) != 8)
            rollback(SBUF("Peggy: Internal error writing vault"), 1);
